...
class Bubble: NSObject {
    ...
    private func firstPriorityModel(priorityArray: inout [BubblePriorityModel]) -> BubblePriorityModel? {
        if !priorityArray.isEmpty, let model = priorityArray.first {
            var passed = true
            if let bound = model.configuration.bubbleView.dataModel.boundViewController, let current = UIWindow().elk.currentViewController {
    ...
    private func deleteNotExistPriority() {
        guard let keyWindow = Bubble.keyWindow() else { return }
        for view in keyWindow.subviews {
            if let bubbleView = view as? BubbleView, !bubbleView.dataModel.existPriority {
