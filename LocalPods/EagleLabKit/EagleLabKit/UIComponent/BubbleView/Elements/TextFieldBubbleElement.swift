...
class TextFieldBubbleElement: BasicBubbleElement {
    ...
    override func setUpDetailBubbleElement() {
        ...
        if let textFieldModel = model as? TextFieldBubbleModel {
            layoutTextFieldElement(textFieldModel: textFieldModel)
        }
    }

    private func layoutTextFieldElement(textFieldModel: TextFieldBubbleModel) {
        ...
        if !textFieldModel.placeHolderContent.isEmpty {
            ...
            let font = BubbleConfig.normalFont(ofSize: fontSize)
        ...
        textField.font = BubbleConfig.normalFont(ofSize: fontSize)
    ...
    override class func elementHeight(model: BasicBubbleModel, elementWidth: CGFloat) -> CGFloat {
        if model is TextFieldBubbleModel {
            return 68.0
        } else {
            return 0.0
        }
