...
class BubbleClassTool {
    ...
    class func getClass(classString: String) -> AnyClass? {
        ...
        if anyClass == nil {
            if let className = NSStringFromClass(self) as String? {
                let targetClassName = className.replacingOccurrences(of: "BubbleClassTool", with: classString)
                anyClass = NSClassFromString(targetClassName)
            }
        }
        return anyClass
    }
