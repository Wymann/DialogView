...
class BubbleTextTool {
    ...
    class func createAttributedString(configuration: AttributedStringConfig) -> NSAttributedString {
        ...
        attributedText.addAttribute(NSAttributedString.Key.font, value: BubbleConfig.normalFont(ofSize: configuration.fontSize), range: range)
                ...
                if richType == "color" {
                    ...
                    if let fontSize: CGFloat = richDic["fontSize"] as? CGFloat {
                        ...
                        if let bold: Bool = richDic["bold"] as? Bool, bold {
                            font = BubbleConfig.boldFont(ofSize: fontSize)
                        } else {
                            font = BubbleConfig.normalFont(ofSize: fontSize)
                        }
