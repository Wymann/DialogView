...
public enum ELKPageControlAlignment {
    case center
    case left
    case right
}

public class ELKPageControl: UIControl {
    /// page数量
    ...
    /// 圆点对齐方式
    public var alignment: ELKPageControlAlignment = .center
    /// 圆点圆角
    ...
    private func getFrame(index: Int) -> CGRect {
        ...
        switch alignment {
        case .center:
            originalX = (frame.size.width - totalWidth) / 2 + spacing
        case .left:
            originalX = spacing
        case .right:
            originalX = frame.size.width - totalWidth + spacing
        }
        var fixedX: CGFloat = 0
        if index <= currentPage {
            fixedX = originalX + CGFloat(index) * itemW
        } else {
            fixedX = originalX + CGFloat(index - 1) * itemW + currentItemW
        }
        ...
        let fixedY = (frame.size.height - height) / 2
        return CGRect(x: fixedX, y: fixedY, width: width, height: height)
    }
