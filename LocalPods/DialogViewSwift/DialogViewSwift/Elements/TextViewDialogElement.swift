...
class TextViewDialogElement: BasicDialogElement {
    ...
    public lazy var textView: DialogTextView = {
    ...
    public lazy var maxLengthLabel: UILabel = {
        let maxLengthLabel = UILabel()
        maxLengthLabel.textColor = UIColor(hexString: DialogConfig.commonColor() + "66")
        maxLengthLabel.font = UIFont.systemFont(ofSize: 14.0)
        maxLengthLabel.textAlignment = NSTextAlignment.right
        return maxLengthLabel
    }()

    public func textViewMaxLength() -> Int {
        if let textViewModel = model as? TextViewDialogModel {
    ...
    private func layoutTextViewElement(textViewModel: TextViewDialogModel) {
        ...
        textView.placeholder = textViewModel.placeHolderContent as NSString
    ...
    override class func elementHeight(model: BasicDialogModel, elementWidth: CGFloat) -> CGFloat {
        if model is TextViewDialogModel {
            return 144.0
        } else {
            return 0.0
        }
