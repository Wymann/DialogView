...
class ImageDialogElement: BasicDialogElement {
    ...
    override func setUpDetailDialogElement() {
        ...
        if let imageModel = model as? ImageDialogModel {
            layoutImageElement(imageModel: imageModel)
        }
    }

    private func layoutImageElement(imageModel: ImageDialogModel) {
        if frame.height > 0 {
            ...
            if imageModel.image != nil {
                ...
                let dialogImageCache = ImageCache(name: "DialogImageCache")
                let options: KingfisherOptionsInfo = [.targetCache(dialogImageCache), .transition(ImageTransition.fade(1))]

                imageView.kf.indicatorType = .activity
                imageView.kf.setImage(with: url, placeholder: imageModel.placeholder, options: options) { result in
                    switch result {
                    case .success:
                        if !imageModel.imageBackColor.isEmpty {
