...
class SelectorDialogItem {
    ...
    public class func createItem(title: String, subtitle: String, selected: Bool, enabled: Bool) -> SelectorDialogItem {
        let item = SelectorDialogItem()
        item.title = title
...
class SelectorDialogModel: BasicDialogModel {
    // 偏好配置
    var preference: SelectorPreference = .init() {
        didSet {
            setupSelectorDialogItemPreference()
        }
    }
    ...
    private func setupSelectorDialogItemPreference() {
        for item: SelectorDialogItem in items {
